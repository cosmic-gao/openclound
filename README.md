...
├── docker-compose.yml      # omniagent 单服务,连外部托管 Postgres + Redis
...
**Docker(本地 / 生产,推荐)**——单 omniagent 容器连外部 pg/redis:
...
1. 代码:`from omniagent import list_skills, write_skill_file, delete_skill`(见 [storage.py](src/omniagent/storage.py));
2. HTTP(见 [http.py](src/omniagent/http.py)):`GET /skills` 与文件级 `GET/PUT/PATCH/DELETE /skills/{name}/files/{path}`(按 `agent=<id>`)。
