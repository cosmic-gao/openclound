...
同一个图 `omniagent` 既能做**完整自由 ReAct**(像 Claude Code),又能做**结构化「检索→规划→执行→审核」管线**;
...
```bash
uv sync --extra aegra        # 安装 aegra-cli(仅部署需要,Python >=3.12)
cp .env.example .env         # 连接 / 模型走 assistant config,无需在此配
uv run aegra dev             # Docker 起 Postgres + 开发服务(http://127.0.0.1:2026,启动自动迁移)
# 生产(自备 / 托管 Postgres + 可选 Redis):
#   DATABASE_URL=… uv run aegra serve     # 启动时自动执行迁移
#   多实例横向扩展:REDIS_BROKER_ENABLED=true REDIS_URL=… aegra serve
```

> 迁移在 `aegra dev` / `aegra serve` **启动时自动执行**(Alembic);Aegra CLI 无 `db` 子命令。
> `aegra dev` / `aegra up` 依赖 Docker;Windows 下生产建议用 Linux / 容器(`aegra up`)。

## 多 Agent / 隔离(无租户)

**两方分工**——agent / 会话走 Aegra 原生,omniagent 只补 skill + 记忆 + 双模式装配:
...
Aegra 仅内网部署,网关在内网注入 `X-User-Id`,[auth.py](src/omniagent/auth.py) 产出 `{identity}`
...
| **backend root**(skill + 运行期文件 + 记忆) | **per-assistant**(`<workspace>/<agent>`) | omniagent 磁盘 + 平台 store |

> 跨资源访问控制由网关 / Aegra `@auth.on` 在入口做;omniagent 只负责 per-assistant 的磁盘 / 记忆隔离。
...
2. HTTP(见 [http.py](src/omniagent/http.py)):`GET/PUT/DELETE /skills?agent=<id>`(PUT body `{"files": {...}}`)。

**删除 agent**:Aegra `DELETE /assistants/{id}` 只删记录、无生命周期钩子,故网关删除成功后应调
`DELETE /agents/{id}`(omniagent),删该 agent 的整个 backend root(`purge_agent`,返回 `{"purged": true}`)。
