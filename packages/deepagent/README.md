...
- 环境变量 `DEEPAGENT_API_KEY`(网关密钥;也兼容 `OPENAI_API_KEY`)
...
│       ├── config.py       # Settings + get_settings(读 DEEPAGENT_* 环境变量)
│       ├── model.py        # build_model():经 litellm 网关接入 ChatOpenAI
│       ├── tools.py        # 示例自定义工具
│       ├── mcp.py          # load_mcp_tools():加载 MCP server 工具
│       ├── memory.py       # build_checkpointer() / build_store()
│       ├── middleware.py   # build_middleware() / high_risk_interrupts()
│       ├── workspace.py    # ensure_workspace():首次从 _data 模板初始化工作区
...
os.environ["DEEPAGENT_API_KEY"] = "..."   # litellm 网关密钥
...
```bash
cp .env.example .env        # 然后编辑 .env,至少填 DEEPAGENT_API_KEY
```
...
| 变量 | 默认 | 说明 |
|---|---|---|
| `DEEPAGENT_BASE_URL` | 网关地址 | litellm 网关 `/v1` 地址 |
| `DEEPAGENT_API_KEY` | `anything` | 网关密钥(兼容 `OPENAI_API_KEY`) |
| `DEEPAGENT_MODEL` | `claude-sonnet-4-6` | 模型名(由网关路由) |
| `DEEPAGENT_TEMPERATURE` | `0.0` | 采样温度 |
| `DEEPAGENT_FALLBACK_MODEL` | 空 | 主模型失败时的备用模型 |
| `DEEPAGENT_WORKSPACE` | `.deepagent` | 工作区目录(agent 的文件系统根) |
| `DEEPAGENT_ENABLE_SHELL` | `1` | 是否启用 `shell` 工具 |
| `DEEPAGENT_ENABLE_FILE_SEARCH` | `0` | 是否启用 ripgrep 文件搜索(deepagents 已有 glob/grep) |
| `DEEPAGENT_ENABLE_HITL` | `0` | 高危工具是否需人工确认 |
| `DEEPAGENT_PII_STRATEGY` | `off` | PII 脱敏策略:`off`/`block`/`redact`/`mask`/`hash` |
| `DEEPAGENT_MODEL_CALL_LIMIT` | 空 | 单次运行模型调用上限 |
| `DEEPAGENT_TOOL_CALL_LIMIT` | 空 | 单次运行工具调用上限 |
| `DEEPAGENT_MODEL_MAX_RETRIES` | `2` | 模型调用最大重试 |
| `DEEPAGENT_TOOL_MAX_RETRIES` | `2` | 工具调用最大重试 |
| `DEEPAGENT_CONTEXT_EDIT_TRIGGER_TOKENS` | `100000` | 触发清理旧工具输出的 token 阈值 |
| `DEEPAGENT_SUMMARIZE_TRIGGER_TOKENS` | 空 | 额外摘要的触发阈值(deepagents 已内置摘要) |

## 工作区与 skills / memory

首次运行时,`ensure_workspace()` 会把随包模板(`_data/`)中的 `skills/` 与
`memories/AGENTS.md` 复制到工作区(默认 `./.deepagent/`)。之后:
...
- **shell 能力(跨平台)**:启用时用 `LocalShellBackend`,其 `execute` 工具基于
  ...
  等环境变量;关闭(`DEEPAGENT_ENABLE_SHELL=0`)时退回 `FilesystemBackend`,
  `execute` 会被自动过滤。不使用 bash-only 的 `ShellToolMiddleware`,以保证跨端可用。
- **记忆**:默认是 **进程内** checkpointer / store,适合开发与单进程 CLI;
  生产可换 Sqlite / Postgres 或交给部署平台注入持久层。
