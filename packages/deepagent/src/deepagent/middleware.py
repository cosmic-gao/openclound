...
(见 :func:`high_risk_interrupts`),而非作为一条 middleware。
"""
...
def build_middleware(
    ...
    *,
    ...
    # —— 健壮性:单次运行的调用上限(防失控) ——
    if settings.model_call_run_limit is not None:
        middleware.append(
            ModelCallLimitMiddleware(
                run_limit=settings.model_call_run_limit, exit_behavior="end"
            )
        )
    if settings.tool_call_run_limit is not None:
        middleware.append(
            ToolCallLimitMiddleware(
                run_limit=settings.tool_call_run_limit, exit_behavior="continue"
            )
        )
    ...
    # 可选:在内置摘要之外再加一条按 token 触发的摘要(默认关闭)。
    if settings.summarize_trigger_tokens is not None:
        middleware.append(
            SummarizationMiddleware(
